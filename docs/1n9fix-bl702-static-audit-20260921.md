# CH32 与 BL702 静态对照（历史基线 668b64e）
...
- BL702 的 CMakeLists.txt（`BL702@1b27d3f:firmware/app/usb2uartjtag/CMakeLists.txt:3`） 指向 `jtag_process.c`；旁边的 `jtag_process.c.gowin` 没有列入该目标。本次按源码内默认开启的 DR32 合并、整批处理及编程页收集宏分析，不声称核实了某块 BL702 板上二进制的编译参数。
...
CMakeLists.txt:61（`CH32@668b64e:CMakeLists.txt:61`） 用 `CACHE ... FORCE` 固定 `BOARD_TYPE=20PINOUT`。即使传入 `-DBOARD_TYPE=22PINOUT`，也会被覆盖；当前分支没有 `boardtype/22pinout.h`。
...
依据：20pinout.h:17（`CH32@668b64e:boardtype/20pinout.h:17`）。22PINOUT 列来自此前已核实的板级配置，并非当前分支文件。
...
依据：CH32 容量定义（`CH32@668b64e:jtag_core/jtagManager.h:12`）、整包背压（`CH32@668b64e:jtag_core/jtagManagerBuffer.c:19`）、环形队列计数（`CH32@668b64e:jtag_core/jtagRingBuffer.c:44`）、USB OUT 邮箱（`CH32@668b64e:usb_bsp/ft232Usbd.c:1304`）、BL702 缓冲区（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:90`）。

BL702 注释描述的编程传输是 1754 B，只能说明其设计针对的一个传输形态，不是所有 Programmer/文件的最大长度保证。若连续传输确实超过 4096 B，两边仍会在容量边界分批；不丢字节并不自动保证 Flash 所需的连续时序。

当前 Link.ld（`CH32@668b64e:Ld/Link.ld:3`） 预留 2048 B 栈，总 RAM 为 10240 B。直接把 RX 改为 8192 而不重新安排内存，仅 `8192 + 512 + 2048 = 10752 B` 就已经超出 RAM，尚未计算 USB、UART 和其它状态。不能把上一分支的 8 KiB 定义单独搬过来，也不能未经依据就削减栈。

**1. 优先级高：2 ms 超时会把未收齐的编程页提前执行**

CH32 所有输入共用同一个批次策略：短包、空间不足 64 B、或距离上一包满 2 ms 即执行。ftdiJtagService.c:254（`CH32@668b64e:jtag_core/ftdiJtagService.c:254`）、超时放行位置（`CH32@668b64e:jtag_core/ftdiJtagService.c:270`）。

BL702 在首包前 32 B 匹配 `4B 03 03 1B 06 71`，识别编程传输后保持 collecting，直到非零短包或容量边界。`jtag_process()` 在 collecting 时直接返回，不存在 2 ms 提前放行。BL702 收包（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:328`）、处理入口（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:415`）。
...
CH32 的 jtag_gpio_edge_delay（`CH32@668b64e:gpio_toggle/GPIO_Cfg.c:410`） 是两条 `nop`。BL702 的 同类延时宏（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:47`） 是两轮 `volatile uint32_t` 计数循环，每轮还有 `nop`。计数、比较、分支及变量访问都会产生额外开销，两个“2”不代表相同的半周期。

编程 DR32 两边都采用单端口整字写，在最后一拍带 TMS 离开 Shift-DR，数据位顺序的基本结构相符；最明确的未对齐项是引脚间隔和处理开销。CH32 DR32（`CH32@668b64e:gpio_toggle/GPIO_Cfg.c:315`）、BL702 DR32（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:177`）。
...
依据：CH32 擦除识别（`CH32@668b64e:jtag_core/jtagGowinFlash.c:96`）、CH32 时钟生成（`CH32@668b64e:gpio_toggle/GPIO_Cfg.c:367`）、BL702 长时钟（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:219`）。
...
CH32 在 TX 无空位时返回 `JTAG_SERVICE_WAIT_TX`，service 随即解锁，再通过 USB 发出部分结果后继续处理。jtagManager.c:299（`CH32@668b64e:jtag_core/jtagManager.c:299`）、service 临界区（`CH32@668b64e:jtag_core/ftdiJtagService.c:122`）。
...
CH32 在 IR71、物理 TAP 位于 Exit1-DR 时，会暂存完整的两拍 `4B 01 01` 或 `4B 01 81`，不立即输出 Update-DR/Idle。它只在收到后续命令、后续 TMS 数据、或输出屏障时补发。DeferProgramExit（`CH32@668b64e:jtag_core/jtagGowinFlash.c:291`）、消费但不输出的分支（`CH32@668b64e:jtag_core/jtagManager.c:310`）、后续命令补发（`CH32@668b64e:jtag_core/jtagManager.c:103`）。
...
在 IR71（也扩展到 IR75 地址）且处于 Shift-DR 时，CH32 一看到长度为 3 的 `0x11/0x19` 就先缓存 24 位、不输出。随后严格要求七位 `0x13/0x1B` 和一位 `0x4B`；中间其它命令会把解析器置为 `JTAG_MPSSE_FAULT`。ProgramCommandValid（`CH32@668b64e:jtag_core/jtagGowinFlash.c:163`）、数据约束（`CH32@668b64e:jtag_core/jtagGowinFlash.c:193`）。
...
CH32 在每轮 service 尾部无条件调用 USB 空闲服务；该服务根据 pending、bit mode、IN 端点空闲及固定 7200000 ticks 判断是否补 `31 60`，不认识 JTAG 批次或未履行退出。service:172（`CH32@668b64e:jtag_core/ftdiJtagService.c:172`）、USB 条件（`CH32@668b64e:usb_bsp/ft232Usbd.c:274`）。

BL702 只有 `jtag_received_flag` 清零后才进入 IN 发送服务。其 JTAG 空包判断是距上次真实数据发送超过 1000 µs；发送空包时不更新 `last_send`，所以不能表述成严格“每 1 ms 一包”。BL702 门控（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:401`）、BL702 定时（`BL702@1b27d3f:firmware/app/usb2uartjtag/main.c:196`）。
...
依据：初始化/循环入口（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:408`）、置 true 与使用（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:610`）、循环跳转（`BL702@1b27d3f:firmware/app/usb2uartjtag/jtag_process.c:899`）。这是条件性静态缺陷；纯编程页的固定形态未必触发。CH32 每次 shift 都重新计算 `suppress_gpio`，没有同样的局部标志残留，不能为追求一致而搬回这个问题。
...
- 常规处理已有全局关中断，并在锁内复查 reset/purge；不是只关 USB 中断。依据：port 临界区（`CH32@668b64e:usb_bsp/ft232MpssePort.c:134`）、CSR 保存恢复（`CH32@668b64e:gpio_toggle/JtagGpioInterrupt.h:13`）。USB 头文件里“只屏蔽 USBD”的注释不能替代实际调用链判断。
