...
void NoData_Setup0(void)
{
  ...
  /* 无数据阶段的标准写请求必须由主机发往设备，拒绝畸形方向位。 */
  if (((pInformation->USBbmRequestType & REQUEST_TYPE) == STANDARD_REQUEST) &&
      ((pInformation->USBbmRequestType & 0x80U) != 0U))
  {
    ...
    else if (Type_Recipient == (STANDARD_REQUEST | ENDPOINT_RECIPIENT))
    {
          ...
          && (Status != 0))
      {
        CopyRoutine = Standard_GetStatus;
           ...
           (pInformation->USBbmRequestType == 0x80U) &&
           (pInformation->USBwValue == 0U) &&
           (pInformation->USBwIndex == 0U) &&
           (pInformation->USBwLength == 1U))
  {
    if (Type_Recipient == (STANDARD_REQUEST | DEVICE_RECIPIENT))
    {
